...
func TestSLOAwareRouter_Dependencies(t *testing.T) {
	predictor := &mockPredictor{}
	router := NewSLOAwareRouter(predictor, HeadroomStrategyLeast)

	deps := router.Dependencies()
	assert.Len(t, deps, 1, "Should have exactly 1 dependency")
	assert.Equal(t, "prefix-cache-scorer", deps[0].Type, "Dependency type should be prefix-cache-scorer")
	assert.Equal(t, "prefix-cache-scorer", deps[0].Name, "Dependency name should be prefix-cache-scorer")
}
